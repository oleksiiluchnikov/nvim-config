...
local function get_models(self)
    ...
    if not adapter then
        log:error(
            'Could not resolve OpenRouter adapter in the `get_models` function'
        )
        return {}
    end
    ...
    if not ok then
        log:error(
            'Could not get the OpenAI compatible models from '
                .. url
                .. models_endpoint
                .. '.\nError: %s',
            response
    ...
    if not ok then
        log:error(
            'Could not parse the response from ' .. url .. models_endpoint
        )
        return {}
    end
    ...
    _cache_expires = utils.refresh_cache(
        _cache_file,
        config.adapters.http.opts.cache_models_for
    )
    ...
    handlers = {
                    ...
                    if item.details and #item.details > 0 then
                        for _, rd in ipairs(item.details) do
                            ...
                            if rd.summary and rd.summary ~= '' then
                                details.summary = (details.summary or '')
                                    .. rd.summary
                            end
        ...
        chat_output = function(self, data, tools)
            ...
            local data_mod = type(data) == 'table' and data.body
                or utils.clean_streamed_data(data)
            local ok, json =
                    ...
                    if delta and delta.tool_calls and #delta.tool_calls > 0 then
                        for i, tool in ipairs(delta.tool_calls) do
                            ...
                            if not id or id == '' then
                                id =
                                    string.format('call_%s_%s', json.created, i)
                            end
            ...
            local result =
                { status = 'success', output = { role = delta.role } }
        ...
        tools = {
            ...
            output_response = function(self, tool_call, output)
                return openai.handlers.tools.output_response(
                    self,
                    tool_call,
                    output
                )
