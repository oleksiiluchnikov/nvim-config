...
M.config = {
    ...
    on_motions = {
        gg = {
            from = function()
                local fg = vim.api.nvim_get_hl(
                    0,
                    { name = 'Comment', create = false, link = false }
                ).fg
        ...
        G = {
            from = function()
                local fg = vim.api.nvim_get_hl(
                    0,
                    { name = 'Conditional', create = false, link = false }
                ).fg
    ...
    default = {
        from = function()
            local colors = {
                Function = vim.api.nvim_get_hl(
                    0,
                    { name = 'Function', create = false, link = false }
                ).fg,
                Character = vim.api.nvim_get_hl(
                    0,
                    { name = 'Character', create = false, link = false }
                ).fg,
                Constant = vim.api.nvim_get_hl(
                    0,
                    { name = 'Constant', create = false, link = false }
                ).fg,
                Conditional = vim.api.nvim_get_hl(
                    0,
                    { name = 'Conditional', create = false, link = false }
                ).fg,
                Define = vim.api.nvim_get_hl(
                    0,
                    { name = 'Define', create = false, link = false }
                ).fg,
                Type = vim.api.nvim_get_hl(
                    0,
                    { name = 'Type', create = false, link = false }
                ).fg,
                DiagnosticError = vim.api.nvim_get_hl(
                    0,
                    { name = 'DiagnosticError', create = false, link = false }
                ).fg,
                DiagnosticOk = vim.api.nvim_get_hl(
                    0,
                    { name = 'DiagnosticOk', create = false, link = false }
                ).fg,
            }
        ...
        to = function()
            ...
            local bg = vim.api.nvim_get_hl(
                0,
                { name = 'CursorLine', create = false, link = false }
            ).bg
...
function beacon:__list_render()
    ...
    local line = vim.api.nvim_buf_get_lines(self.buffer, Y, Y + 1, false)[1]
        or ''
...
function beacon:render()
    if
        type(self.window) == 'number' and vim.api.nvim_win_is_valid(self.window)
    then
        self:__list_render()
