...
highlights.set_hl = function(name, value)
    ...
    local found, v = pcall(
        vim.api.nvim_get_hl,
        0,
        { name = name, create = false, link = false }
    )
...
highlights.get_property = function(property, groups, light, dark)
    ...
    for _, item in ipairs(groups) do
        local hl = vim.api.nvim_get_hl(
            0,
            { name = item, link = false, create = false }
        )
...
highlights.inherit = function(from, with, properties)
    ...
    local _from = vim.api.nvim_get_hl(
        0,
        { name = from, link = false, create = false }
    ) or {}
    local output = {}
...
highlights.groups = {
    FadedBg = function()
        ...
        local alpha = vim.g.faded_alpha
            or (vim.o.background == 'light' and 0.25 or 0.5)
        ...
        return {
            bg = highlights.rgb_to_hex(highlights.oklab_to_srgb(faded)),
        }
    ...
    Diagnostic = function()
        ...
        local function handle_kind(kind)
            ...
            local fg = highlights.srgb_to_oklab(highlights.get_property('fg', {
                kind == 'Default' and '@comment'
                    or string.format('Diagnostic%s', kind),
            ...
            table.insert(output, {
                ...
                value = {
                    ...
                    fg = highlights.rgb_to_hex(highlights.oklab_to_srgb(fg)),
                },
            })
            table.insert(output, {
                group_name = string.format('FancyDiagnostic%sIcon', kind),
                value = {
                    bg = highlights.rgb_to_hex(highlights.oklab_to_srgb(fg)),
                    fg = highlights.rgb_to_hex(highlights.oklab_to_srgb(bg)),
                },
            })
    ...
    InspectTree = function()
        ...
        local function handle_level(n)
            ...
            local fg = highlights.srgb_to_oklab(highlights.get_property('fg', {
                n == 0 and '@comment'
                    or string.format('@markup.heading.%d.markdown', n),
            }, this_color[1], this_color[2]))
            local new_bg = highlights.mix(fg, bg, alpha, 1 - alpha)
