...
diagnostics.config = {
    ...
    beacon = {
        ...
        default = {
            ...
            to = function()
                local bg = vim.api.nvim_get_hl(0, {
                    name = vim.o.statusline and 'Cursorline' or 'Normal',
                    link = false,
                }).bg
        ...
        [vim.diagnostic.severity.INFO] = {
            from = function()
                local fg = vim.api.nvim_get_hl(
                    0,
                    { name = 'FancyDiagnosticInfoIcon', link = false }
                ).fg
        ...
        [vim.diagnostic.severity.HINT] = {
            from = function()
                local fg = vim.api.nvim_get_hl(
                    0,
                    { name = 'FancyDiagnosticHintIcon', link = false }
                ).fg
                return fg and string.format('#%06x', fg) or '#94e2d5'
            end,
        },
        [vim.diagnostic.severity.WARN] = {
            from = function()
                local fg = vim.api.nvim_get_hl(
                    0,
                    { name = 'FancyDiagnosticWarnIcon', link = false }
                ).fg
        ...
        [vim.diagnostic.severity.ERROR] = {
            from = function()
                local fg = vim.api.nvim_get_hl(
                    0,
                    { name = 'FancyDiagnosticErrorIcon', link = false }
                ).fg
    ...
    decorations = {
        ...
        [vim.diagnostic.severity.INFO] = handle_diagnostic_level(
            'Info',
            '󰀨 '
        ),
        [vim.diagnostic.severity.HINT] = handle_diagnostic_level(
            'Hint',
            '󰁨 '
        ),
        [vim.diagnostic.severity.WARN] = handle_diagnostic_level(
            'Warn',
            ' '
        ),
        [vim.diagnostic.severity.ERROR] = handle_diagnostic_level(
            'Error',
            '󰅙 '
        ),
...
local function virt_text_to_sign(virt_text)
    ...
    for _, item in ipairs(virt_text) do
        if type(item[2]) == 'string' then
            output = output
                .. string.format('%%#%s#%s', item[2], item[1])
                .. '%#Normal#'
        else
...
diagnostics.__integration = function(window, beacon_config)
    ...
    if package.loaded['scripts.beacon'] then
        if not diagnostics.__beacon then
            diagnostics.__beacon =
                require('scripts.beacon').new(window, beacon_config)
        else
...
diagnostics.hover = function(window)
    ...
    local already_open = diagnostics.window
        and vim.api.nvim_win_is_valid(diagnostics.window)
    ...
    if
        not diagnostics.window
        or not vim.api.nvim_win_is_valid(diagnostics.window)
    then
        diagnostics.window =
            vim.api.nvim_open_win(diagnostics.buffer, false, height_calc_config)
    else
    ...
    local beacon_config = vim.tbl_extend(
        'force',
        get_beacon_config('default', {}, true),
        get_beacon_config(level, {}, true) or {}
    )

    local H = vim.api.nvim_win_text_height(
        diagnostics.window,
    ...
    vim.wo[diagnostics.window].statuscolumn =
        '%!v:lua.fancy_diagnostics_statuscolumn()'
